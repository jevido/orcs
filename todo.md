...
So i want to write in a Laravel fashion, however i want to write a sort of compiler. So it runs in Bun and Javascript in general.
However we are not a full stack framework. We're backend only. Everything works in javascript, and JSON should be parsed before the request route is handled.

## Routes

The routes should be bun compatible. it should not use the fetch, but my routes should be transformed to routes bun can understand directly. It is used like this:
